...
namespace CarBook.Web.UI.Areas.Admin.Controllers
{
    ...
    public class AdminLocationController : Controller
    {
        ...
        public async Task<IActionResult> CreateLocation(CreateLocationDto createLocationDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7262/api/Location", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7262/api/Location/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7262/api/Location/{id}");
        ...
        public async Task<IActionResult> UpdateLocation(UpdateLocationDto updateLocationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7262/api/Location/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
