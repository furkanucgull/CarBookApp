...
namespace CarBook.Web.UI.Areas.Admin.Controllers
{
    ...
    public class AdminPricingController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7262/api/Pricing");
        ...
        public async Task<IActionResult> CreatePricing(CreatePricingDto createPricingDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7262/api/Pricing", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7262/api/Pricing/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var resposenMessage = await client.GetAsync($"https://localhost:7262/api/Pricing/{id}");
        ...
        public async Task<IActionResult> UpdatePricing(UpdatePricingDto updatePricingDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7262/api/Pricing/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
