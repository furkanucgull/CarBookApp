...
namespace CarBook.Web.UI.ViewComponents.BlogViewComponents
{
    public class _BlogDetailsTagCloudComponentPartial : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7262/api/TagClouds/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<GetByBlodIdTagCloudDto>(jsonData);
                return View(values);
            }
            return View();
        }
